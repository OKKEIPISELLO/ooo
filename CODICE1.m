...
% --- Calcolo area e semiassi ellisse dati Fx,Fy ---
function [area, a, b] = area_ellisse(Fx, Fy)
    pts = [Fx(:), Fy(:)];
    C = cov(pts);
    [V,D] = eig(C);
    % semiassi = deviazione standard moltiplicata per 2 (per coprire l'intera ellisse)
    a = 2*sqrt(D(2,2));
    b = 2*sqrt(D(1,1));
    area = pi*a*b/4; % area = pi*a*b/4 (perché a,b sono diametri)
end
...
function animazione_pushdown_separata(t1, P1_1, P2_1, P3_1, theta1_1, theta2_1, J1, tau1, ...
    ...
    % --- TABELLE per AREA e DIAGONALE MAGGIORE ---
    uitblArea = uitable('Data',{'--','--'},'ColumnName',{'errata','corretta'},...
        'Position',[20 400 200 48],'FontSize',12,'BackgroundColor',[0.9 1 0.9]);
    uicontrol('Style','text','String','Area ellisse di forza [N^2]','Position',[20 470 200 40],'FontSize',12,'BackgroundColor',[0.9 1 0.9]);

    uitblDiagLen = uitable('Data',{'--','--'},'ColumnName',{'errata','corretta'},...
        'Position',[20 330 200 48],'FontSize',12,'BackgroundColor',[0.9 1 1]);
    uicontrol('Style','text','String','Lunghezza diagonale maggiore [N]','Position',[20 400 200 40],'FontSize',12,'BackgroundColor',[0.9 1 1]);
    ...
    function aggiornaEllisse(frame)
        ...
        if all(~isnan(Fx1))
            ...
            % --- AREA e LUNGHEZZA DIAGONALE ---
            [area1, a1, ~] = area_ellisse(Fx1, Fy1);
            areaData = get(uitblArea,'Data');
            areaData{1,1} = sprintf('%.2f',area1);
            set(uitblArea,'Data',areaData);

            diagData = get(uitblDiagLen,'Data');
            diagData{1,1} = sprintf('%.2f',2*a1);
            set(uitblDiagLen,'Data',diagData);
        ...
        if all(~isnan(Fx2))
            ...
            [area2, a2, ~] = area_ellisse(Fx2, Fy2);
            areaData = get(uitblArea,'Data');
            areaData{1,2} = sprintf('%.2f',area2);
            set(uitblArea,'Data',areaData);

            diagData = get(uitblDiagLen,'Data');
            diagData{1,2} = sprintf('%.2f',2*a2);
            set(uitblDiagLen,'Data',diagData);
